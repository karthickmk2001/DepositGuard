  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm">{tenancy.property_address}</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl divide-y divide-slate-800 mb-6">
        ...
        <div className="px-5 py-3.5 flex justify-between">
          <span className="text-slate-400 text-sm">Landlord</span>
          <span className="font-mono text-xs text-slate-300">{tenancy.landlord.slice(0, 8)}...{tenancy.landlord.slice(-8)}</span>
        </div>
        <div className="px-5 py-3.5 flex justify-between">
          <span className="text-slate-400 text-sm">Tenant</span>
          <span className="font-mono text-xs text-slate-300">{tenancy.tenant ? `${tenancy.tenant.slice(0, 8)}...${tenancy.tenant.slice(-8)}` : "—"}</span>
        </div>
        <div className="px-5 py-3.5 flex justify-between">
          <span className="text-slate-400 text-sm">Deposit in escrow</span>
          <span className="font-bold text-teal-400 tabular-nums">{tenancy.deposit_amount} DEPG</span>
        </div>
        <div className="px-5 py-3.5 flex justify-between">
          <span className="text-slate-400 text-sm">Landlord originally proposed</span>
          <span className="text-orange-400">{tenancy.proposed_landlord_amt ?? "—"} DEPG to themselves</span>
        </div>
        <div className="px-5 py-3.5 flex justify-between">
          <span className="text-slate-400 text-sm">Inspection tier</span>
      ...
      <div className="space-y-3 mb-6">
        {tenancy.move_in_hash && (
          <div className="bg-slate-900 border border-teal-800/40 rounded-xl px-5 py-4">
            <div className="text-xs text-teal-400 font-medium mb-1">Move-in photo hash (mutually signed)</div>
            <div className="font-mono text-xs text-slate-300 break-all">{tenancy.move_in_hash}</div>
          </div>
        )}
        {tenancy.inspector_move_in_hash && (
          <div className="bg-slate-900 border border-green-800/40 rounded-xl px-5 py-4">
        ...
        {tenancy.move_out_hash && (
          <div className="bg-slate-900 border border-orange-800/40 rounded-xl px-5 py-4">
        ...
        {tenancy.inspector_move_out_hash && (
          <div className="bg-slate-900 border border-green-800/40 rounded-xl px-5 py-4">
            ...
            <div className="font-mono text-xs text-slate-300 break-all">{tenancy.inspector_move_out_hash}</div>
          </div>
        )}
        {!tenancy.move_in_hash && !tenancy.inspector_move_in_hash && (
          <div className="text-slate-500 text-sm text-center py-4">No on-chain evidence recorded</div>
        )}
      </div>
      ...
      {(hasMoveInPhotos || hasMoveOutPhotos) && (
        <div className="mb-8">
          ...
          {comparisonTab === "move-in" && !hasMoveInPhotos && (
            <p className="text-slate-500 text-sm text-center py-4">No move-in photos</p>
          )}
          {comparisonTab === "move-out" && !hasMoveOutPhotos && (
            <p className="text-slate-500 text-sm text-center py-4">No move-out photos</p>
          )}
        </div>
      )}
        ...
        <>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5">Landlord receives (DEPG)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5">Tenant receives (DEPG)</label>
                <input
                  type="number"
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 placeholder-slate-600 focus:outline-none focus:border-teal-500 transition-colors text-sm resize-none"
              />
            </div>
            ...
            <button
              ...
              className="w-full bg-teal-600 hover:bg-teal-500 disabled:bg-slate-800 disabled:text-slate-500 text-white font-semibold py-4 rounded-xl transition-colors flex items-center justify-center gap-2"
            >
