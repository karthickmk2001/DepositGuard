...
export default function ResolvePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      {/* Escrow PDA */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-3.5 mb-6 flex justify-between items-center">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl divide-y divide-gray-800 mb-6">
          {tenancy.move_in_hash && (
            <div className="px-5 py-3.5">
              <div className="text-xs text-violet-400 font-medium mb-1">Move-in evidence hash</div>
      ...
      {(hasMoveInPhotos || hasMoveOutPhotos) && (
        <div className="mb-8">
          ...
          {comparisonTab === "move-in" && !hasMoveInPhotos && (
            <p className="text-gray-500 text-sm text-center py-4">No move-in photos uploaded</p>
          )}
          {comparisonTab === "move-out" && !hasMoveOutPhotos && (
            <p className="text-gray-500 text-sm text-center py-4">No move-out photos yet</p>
          )}
        </div>
      )}
      ...
      {isLandlord && tenancy.status === "Active" && (
        <div className="space-y-5">
          ...
          <div>
            ...
            <label className="block w-full border-2 border-dashed border-gray-700 hover:border-orange-500 rounded-xl p-6 text-center cursor-pointer transition-colors">
              <input
                type="file"
                multiple
                accept="image/*"
              ...
              <div className="mt-3 space-y-1">
                {moveOutHashes.map((h, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs text-gray-500">
                    ...
                    <span className="text-gray-600">{moveOutPhotos[i]?.name}</span>
                  </div>
                ))}
                {moveOutCombinedHash && (
                  <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-4 py-3 mt-2">
                    ...
                    <p className="font-mono text-xs text-gray-300 break-all">{moveOutCombinedHash}</p>
                  </div>
                )}
              </div>
            )}
      ...
      {tenancy.status === "Completed" && (
        <div className="bg-green-900/20 border border-green-700/40 rounded-xl px-5 py-6 text-center">
          ...
          <div className="text-sm text-gray-400 mt-1">Both parties agreed. Escrow has been released.</div>
        </div>
      )}
      ...
      {tenancy.status === "Disputed" && (
        <div className="bg-red-900/20 border border-red-700/40 rounded-xl px-5 py-6 text-center">
          ...
          <a
            href={`/arbitrate/${id}`}
            className="inline-block mt-4 text-sm text-violet-400 hover:text-violet-300 underline"
          >
