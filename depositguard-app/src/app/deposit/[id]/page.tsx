...
export default function DepositPage() {
  ...
  if (!connected) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <p className="text-teal-300 font-medium mb-8">{tenancy.property_address}</p>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      {/* Header */}
      <div className="mb-8">
        <div className="text-sm text-teal-400 font-medium mb-1">Deposit Escrow</div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl divide-y divide-slate-800 mb-6">
        ...
        <div className="px-6 py-4 flex justify-between items-center">
          ...
          <span className="font-bold text-teal-400 text-lg tabular-nums">{tenancy.deposit_amount} DEPG</span>
        </div>
        <div className="px-6 py-4 flex justify-between items-center">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl px-5 py-4 mb-6">
        <p className="text-sm font-medium text-slate-400 mb-1">Escrow PDA (on-chain vault)</p>
      ...
      {tenancy.move_in_hash && (
        <div className="bg-teal-900/20 border border-teal-500/30 rounded-xl px-5 py-4 mb-6">
          <p className="text-sm font-medium text-teal-400 mb-1">Move-in evidence hash (on-chain)</p>
          <p className="font-mono text-xs text-slate-300 break-all">{tenancy.move_in_hash}</p>
      ...
      {tenancy.inspector_move_in_hash && (
        <div className="bg-green-900/20 border border-green-700/40 rounded-xl px-5 py-4 mb-6">
          ...
          <p className="font-mono text-xs text-slate-300 break-all">{tenancy.inspector_move_in_hash}</p>
          {tenancy.inspector && (
            <p className="text-xs text-slate-500 mt-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            {tenancy.move_in_photos.map((url, i) => (
              ...
              <img
                ...
                className="rounded-lg aspect-square object-cover w-full border border-slate-800"
              />
            ))}
          </div>
        </div>
      ...
      {isAlreadyTenant ? (
        <div className="bg-green-900/20 border border-green-700/40 rounded-xl px-5 py-4 text-center">
          ...
          <div className="text-sm text-slate-400 mt-1">Status: {tenancy.status}</div>
        </div>
      ) : isLandlord ? (
        <div className="bg-slate-900 border border-slate-700 rounded-xl px-5 py-4 text-center text-slate-400 text-sm">
        ...
        <div>
          <button
            ...
            disabled={paying}
            className="w-full bg-teal-600 hover:bg-teal-500 disabled:bg-teal-800 text-white font-semibold py-4 rounded-xl transition-colors text-lg flex items-center justify-center gap-3"
          >
            {paying ? (
              <>
